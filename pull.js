// Based on alainbryden original scripts
 ...
 * Allows to delete all 'js, 'scripts', 'ns', and 'txt' files 
 * 
 ...
 * TODO: Some way to list all files in the repository and/or download them all. **/
 export async function main(ns) {
    ...
    if (confirmation) {
        const deleteConfirmation = await ns.prompt('Delete files? \nThis will delete all extra js, ns, script and txt files that are not part of the repository. ')
        if (deleteConfirmation) {
            let lists = await ns.ls("home");
            let filteredList = lists.filter(file => ['txt', 'ns', 'js', 'script'].includes(file.split('.').pop()));
