...
export function growPercent(server, threads, player, cores = 1) {
    ...
    const numServerGrowthCyclesAdjusted = numServerGrowthCycles * serverGrowthPercentage * 1 // BitNodeMultipliers.ServerGrowthRate;
...
function utilCalculateHackingTime(server, player) {
    ...
    const hackingTime = (hackTimeMultiplier * skillFactor) / (player.hacking_speed_mult * utilCalculateIntelligenceBonus(player.intelligence, 1));
    return hackingTime;
}
